...
createStorageBufferDescriptorSet(const vk::raii::Device &device,
    ...
    vk::DescriptorSetLayoutBinding binding{
        0, vk::DescriptorType::eStorageBuffer, 1, stages};
...
testSwapchainAccessors(const std::shared_ptr<devices::WindowInfo> &windowInfo,
    ...
    checkMsg(!sc.needRecreation(),
             "needRecreation() should be false initially");
...
static void testShaderManager(Backend &backend) {
    ...
    if (result) {
        ...
        auto mod = backend.getShaderManager().getModule(&g_shader,
                                                        dev->getDevicePtr());
...
testPipelineManager(Backend &backend,
    ...
    if (!pipeResult) {
        ...
        checkMsg(*pipeResult != nullptr,
                 "createPipeline(dynamic) returned null");
    ...
    // --- Static pipeline ---
    {
        ...
        staticInfo.inputAssembly.topology =
    ...
    if (!compResult) {
        ...
        checkMsg(*compResult != nullptr,
                 "createPipeline(compute) returned null");
        // Cache test
        auto compResult2 = backend.createPipeline(compInfo);
        checkMsg(compResult2.has_value(),
                 "second compute createPipeline failed");
...
static void testExplicitDeviceWindow(Backend &backend, GLFWwindow *glfwWin,
    ...
    if (glfwCreateWindowSurface(**instancePtr, glfwWin, nullptr,
                                &rawSurface2) != VK_SUCCESS) {
        std::cout
            << "[SKIP] testExplicitDeviceWindow: surface creation failed\n";
        return;
    }
...
static void testComputeDispatch(Backend &backend) {
    ...
    // 6. Record compute commands
    {
        ...
        cmdPool.allocate(1);
        vk::CommandBuffer cmd = *cmdPool.buffers[0];
    ...
    devices::transfer(dev, storageBuf, 0, staging, 0, bufferSize);
    staging.invalidate();
...
static void testComputeWithGraphicsSingleShader(
    ...
    // ── Fill the buffer with a compute dispatch ────────────────────────
    {
        ...
        barrier2.dstAccessMask = vk::AccessFlagBits2::eMemoryRead |
...
static void testComputeWithGraphicsMultipleShaders(
    ...
    auto compResult = backend.createPipeline(compInfo);
    checkMsg(compResult.has_value(),
             "multiple shaders: compute pipeline failed");
    auto compPipe = *compResult;
    ...
    vk::raii::PipelineLayout gfxPipelineLayout{*dev->getDevicePtr(),
                                               gfxLayoutCI};
    ...
    auto gfxResult = backend.createPipeline(dynInfo);
    checkMsg(gfxResult.has_value(),
             "multiple shaders: graphics pipeline failed");
    auto gfxPipe = *gfxResult;
    ...
    for (int i = 0; i < 120; ++i) {
        ...
        for (auto &wf : ctx.windows) {
            if (!wf.valid) {
                ...
                vk::Viewport vp{
                    0, 0, (float)wf.extent.width, (float)wf.extent.height,
                    0, 1};
                cmd.setViewport(0, vp);
                cmd.setScissor(0, vk::Rect2D{{0, 0}, wf.extent});
                cmd.draw(3, 1, 0, 0);
...
int main() {
    try {
        ...
        if (glfwCreateWindowSurface(**instancePtr, window, nullptr,
