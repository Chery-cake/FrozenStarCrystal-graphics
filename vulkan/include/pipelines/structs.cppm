...
struct FROZENSTARCRYSTAL_GRAPHICS_API InputAssemblyState {
  ...
  constexpr auto operator<=>(const InputAssemblyState &) const noexcept =
      default;
};
struct FROZENSTARCRYSTAL_GRAPHICS_API RasterizationState {
  ...
  constexpr auto operator<=>(const RasterizationState &) const noexcept =
      default;
};
struct FROZENSTARCRYSTAL_GRAPHICS_API DepthStencilState {
  ...
  constexpr auto operator<=>(const DepthStencilState &) const noexcept =
      default;
};
...
struct FROZENSTARCRYSTAL_GRAPHICS_API AttachmentFormats {
  ...
  bool operator==(const AttachmentFormats &) const noexcept = default;
};
...
struct FROZENSTARCRYSTAL_GRAPHICS_API DynamicPipelineInfo {
  ...
  bool operator==(const DynamicPipelineInfo &) const noexcept = default;
};
...
struct FROZENSTARCRYSTAL_GRAPHICS_API StaticPipelineInfo {
  ...
  bool operator==(const StaticPipelineInfo &) const noexcept = default;
};
...
struct hash<graphics::vulkan::pipelines::PipelineTag<Kind>> {
  ...
  size_t operator()(
    ...
    mix(std::hash<VkPipelineLayout>{}(
        static_cast<VkPipelineLayout>(k.layout)));
...
template <> struct hash<graphics::vulkan::pipelines::DynamicPipelineInfo> {
  ...
  size_t operator()(const graphics::vulkan::pipelines::DynamicPipelineInfo &k)
      const noexcept {
    size_t seed = std::hash<graphics::vulkan::pipelines::DynamicPipeline>{}(
        k.tag);
    ...
    mix(seed, std::hash<uint32_t>{}(static_cast<uint32_t>(k.attachments.depth)));
    mix(seed,
...
template <> struct hash<graphics::vulkan::pipelines::StaticPipelineInfo> {
  size_t operator()(const graphics::vulkan::pipelines::StaticPipelineInfo &k)
      const noexcept {
    ...
    size_t seed =
        std::hash<graphics::vulkan::pipelines::DynamicPipelineInfo>{}(
            dynamicEquivalent);
    constexpr size_t golden = 0x9e3779b9;
    seed ^=
        std::hash<VkRenderPass>{}(static_cast<VkRenderPass>(k.renderPass)) +
        golden + (seed << 6) + (seed >> 2);
    seed ^= std::hash<uint32_t>{}(k.subpass) + golden + (seed << 6) + (seed >> 2);
    return seed;
  }
};
